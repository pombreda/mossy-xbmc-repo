...
class EpisodeList:
	...
	def initialise(self, showId, showTitle):
		...
		try:
			...
			if isinstance(jsonData['feed']['entry'], list):
				self.entries = jsonData['feed']['entry']
			else:
	...
	def createNewListItem(self, episodeDetail):
		newListItem = xbmcgui.ListItem( episodeDetail.label )
		newListItem.setThumbnailImage(episodeDetail.thumbnail)
...
class EpisodeDetail:
	...
	def refine(self, showId, showTitle):
		if ( self.seriesNum == u"" or self.epNum == u"" ):
			...
			self.log("Searching for seasonnnn and episode numbers in url: %s" % self.url)
