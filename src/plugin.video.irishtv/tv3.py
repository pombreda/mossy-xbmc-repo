...
#TODO Check that proxy is used/not used
...
#		self.cache = cache
	...
	def ShowRootMenu(self):
		...
		try:
			html = self.cache.GetURLFromCache(rootMenuUrl, 300).decode('latin1')
	
			if html is None or html == '':
				# Error getting %s Player "Home" page
				logException = LoggingException(logMessage = self.language(20000) % self.GetProviderId())
				# 'Cannot show TV3 root menu', Error getting TV3 Player "Home" page
				logException.process(self.language(20010) % self.GetProviderId(), self.language(20000) % self.GetProviderId(), logLevel)
				#raise logException
				return False
			...
			if len(categories) == 0:
				...
				logException = LoggingException(logMessage = self.language(20020))
...
#			self.AddFeatured(listItems, html)		
	...
	def ParseCommand(self, mycgi):
		...
		try:
			...
			html = self.cache.GetURLFromCache( urlRoot + page, 1800 )
...
#==============================================================================
	...
	def AddAllShowListItem(self, title, video, listItems, thumbnail = None):
		...
		description = self.fullDecode(video.findAll('a')[1].text.decode('latin1'))
	...
	def ListAToZ(self, page = None):
		...
		try:
			...
			html = self.cache.GetURLFromCache(allShowsUrl, 1800).decode('latin1')
...
#==============================================================================
	...
	def PlayEpisode(self, page, html):
		...
		if ageCheck is not None:
			...
			try:
				html = self.cache.GetURLFromCache( urlRoot + page, 1800, values = {'age_ok':'1'} ).decode('latin1')
				soup = BeautifulSoup(html)
			except (Exception) as exception:
				exception = LoggingException.fromException(exception)
		...
		return self.PlayOrDownloadEpisode(infoLabels, thumbnail, rtmpVar)


	def Play(self, infoLabels, thumbnail, rtmpVar):
		self.log ('Play titleId: ' + infoLabels['Title'])
	...
	def PlayOrDownloadEpisode(self, infoLabels, thumbnail, rtmpVar):
...
#		headers = {'DNT':'1', 'X-Requested-With':'XMLHttpRequest' }
		...
		html = self.cache.GetURLFromCache( searchUrl, 1800, values = values, headers = headers ).decode('latin1')
		if html is None or html == '':
	...
	def ListByDate(self, date):
		...
		html = self.cache.GetURLFromCache( calendarUrl, 3600, values = values).decode('latin1')
		if html is None or html == '':
			# Data returned from web page: %s, is: '%s'
			logException = LoggingException(logMessage = self.language(30060) % ( searchUrl, html ))
